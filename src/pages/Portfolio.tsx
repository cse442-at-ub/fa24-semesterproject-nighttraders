...
import { Chart as ChartJS, CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend } from 'chart.js'; // Chart.js components
...
const Portfolio: React.FC = () => {
    ...
    const handleSave = async (symbol: string) => {
        ...
        setOwnedStocks(prev =>
            prev.map(stock =>
                stock.symbol === symbol ? { ...stock, quantity: quantities[symbol] || 0 } : stock
            )
        );
        ...
        try {
            const response = await fetch(`${config.backendUrl}/updateOwnedStocks.php`, {
                ...
                body: JSON.stringify({ symbol, quantity: quantities[symbol] || 0 }),
            });
            const data = await response.json();
            if (data.error) {
                console.error(data.error);
